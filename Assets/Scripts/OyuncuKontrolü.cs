...
public class OyuncuKontrolü : MonoBehaviour
{
    ...
    [SerializeField] GameObject kaybetmeEkrani, kazanmaEkrani;
    ...
    private void OnCollisionEnter2D(Collision2D collision) //oncollision= çarpışma anında
    {
        ...
        {
            GetComponent<zamankontrol>().enabled = false; // düsmanla temas edildiğinde süreyle ilgili kontroller durur.
            Ölüm();
        }
        else if (collision.gameObject.CompareTag("Bitir"))
        {
            /* kazanmaEkrani.SetActive(true); // karakter bitir etiketli objeye temas ettiğinde kazanma ekranı aktifleşir.
             Time.timeScale = 0; */ // timescale oyunu durdurmayı veya yavaşlatmayı sağlar.
            Destroy(collision.gameObject); // oyuncu bitir etiketli objeye çarpınca obje kaybolsun
            StartCoroutine(Geciktir(true)); // oyuncu bitir etiketli objeyle temas ettiğinde geciktir fonksiyonu devreye girer.
        }
    }
    ...
    public void Ölüm()
    {
        ...
        oyuncuBeden.constraints = RigidbodyConstraints2D.FreezeAll; // oyuncu öldüğünde rotasyon hareketleri inaktif olsun
        enabled = false; // karakter ölünce scripts devre dışı kalır
        StartCoroutine(Geciktir(false)); // geciktir fonksiyonunun "else" bölümünü çalıştıracak.
    }
    #endregion Kodların her seferinde tekrarlanmaması için fonksiyon haline getirdik..

    #region kazanma ekranının gösterilmesini geciktirme fonksiyonu.
    IEnumerator Geciktir(bool kazanma)
    {
        yield return new WaitForSecondsRealtime(2f); // yield ertelemeyi sağlar. ve alt kod satırındaki işlemlerin yapılmasını sağlıyor.
        Time.timeScale = 0; // timescale oyunu durdurmayı veya yavaşlatmayı sağlar.
        if (kazanma==true)
        {
        ...
        {
            if (LifeManager.globalErisim.kalanCan==1)
            {
                kaybetmeEkrani.SetActive(true); // karakter öldüğünde kaybetme ekranı aktifleşir.
                LifeManager.globalErisim.kalanCan = 3;
            }
            else
            {
                LifeManager.globalErisim.CanKontrol();
